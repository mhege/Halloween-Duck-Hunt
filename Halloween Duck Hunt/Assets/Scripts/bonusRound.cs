...
public class bonusRound : MonoBehaviour
{
    ...
    public int ghostNumber = 50;//number ghosts per round
    ...
    //consistency. Dog needs time
    IEnumerator roundOverDelay(float wait)
    {
        yield return new WaitForSeconds(wait);
        roundDelay = true;
    }
    ...
    void Update()
    {
        ...
        if (roundDone)//changed
        {
            ...
            scoreScript.missedTarget(numberMisses);

            StartCoroutine(roundOverDelay(2.5f));
